...
def loadDataSet(filename):
    ...
    for line in fr.readlines():
        ...
        dataMat.append([float(lineArr[0:num-1])])  # 数据
        labelMat.append(float(lineArr[num-1]))     # 标签
...
def kernelTrans(X, A, kTup):
    """定义核函数(输入参数X,支持向量的特征树A;某一行特征数据kTup：('lin',k1)核函数的类型和参数)"""
    m,n = shape(X)
    K = mat(zeros((m,1)))
    if kTup[0]=='lin':  # 线性函数
        K = X * A.T
...
#SMO函数，用于快速求解出alpha
def smoP(dataMatIn, classLabels, C, toler, maxIter,kTup=('lin', 0)): #数据，数据类别，参数C，阀值toler，最大迭代次数，核函数（默认线性核）
